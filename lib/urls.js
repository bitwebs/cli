...
//                   1          2      3        4
const VERSION_REGEX = /^(bit:\/\/)?([^/]+)(\+[^/]+)(.*)$/i
export function parseBitUrl (str, parseQS) {
  ...
  if (!parsed.origin) parsed.origin = `bit://${parsed.hostname}/`
  return parsed
}
...
export function fromURLToKeyStr (url) {
  try { 
    ...
    throw new Error(`Invalid bit:// URL, ${url}`)
  }
}

export function fromPathToBittreeKeyList (path) {
