...
import bitwebCrypto from '@web4/bitweb-crypto'
import * as bit from '../bit/index.js'
import * as BitStruct from '../bit/struct.js'
import * as netStats from '../bit/net-stats.js'

export class BitStructInfoTracker {
  constructor (keyStr) {
    this.keyStr = keyStr
    this.key = Buffer.from(keyStr, 'hex')
    this.discoveryKey = bitwebCrypto.discoveryKey(this.key)
  ...
  async fetchState () {
    this.netCfg = await bit.getClient().network.status(this.discoveryKey).catch(e => undefined)
  ...
  async attemptLoadStruct () {
    ...
    this.loadStructPromise = new Promise(async (resolve, reject) => {
      try {
        this.struct = await BitStruct.get(this.keyStr, {noNetConfig: true, expect: undefined})
  ...
  async calcBlockState () {
    if (!this.struct) return
    if (this.struct.type === 'bitdrive') {
      let [total, size, downloaded] = [0, 0, 0]
      let contentChain = await new Promise(resolve => this.struct.api.getContent((err, chain) => resolve(chain)))
      for (let entry of await this.struct.api.promises.readdir('/', {recursive: true, includeStats: true})) {
        total += entry.stat.blocks
        size += entry.stat.size
        downloaded += await countHasChainBlocks(contentChain, entry.stat.offset, entry.stat.offset + entry.stat.blocks)
      }
      this.totalBlocks = total
      this.size = size
      this.downloadedBlocks = downloaded
    } else if (this.struct.type === 'bittree') {
...
async function countHasChainBlocks (chain, from, to) {
  var count = 0
  for (let i = from; i < to; i++) {
    count += (await chain.has(i)) ? 1 : 0
  }
  return count
}
