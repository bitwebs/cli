...
Examples:
  ...
  bit drive sync bit://1234..af/ ./local-folder
  bit drive sync ./local-folder bit://1234..af/remote-folder --no-delete
  bit drive sync bit://1234..af/ bit://fedc..21/
  bit drive sync bit://1234...af/
  bit drive sync ./local-folder
`

export default {
  name: 'drive sync',
  description: 'Continuously sync changes between two folders in your local filesystem or in bitdrives.',
  usage: {
  ...
  command: async function (args) {
    ...
    if (!args._[1]) console.error('Creating new bitdrive...')
    ...
    if (!rightArgs.isBit) mkdirp.sync(rightArgs.path)
    if (!ok) process.exit(0)
    console.error(live ? 'Live syncing (Ctrl+c to exit)...' : 'Syncing...')

    var statusLines = ['']
    var statusLog = statusLogger(statusLines)
    if (leftArgs.isBit) await setupTracker(leftArgs.fs.key.toString('hex'), statusLines, statusLog)
    if (rightArgs.isBit) await setupTracker(rightArgs.fs.key.toString('hex'), statusLines, statusLog)
...
function isUrl (str) {
  return str.startsWith('bit://') || /^[0-9a-f]{64}/.test(str)
}

async function parseArgs (str) {
  if (isUrl(str)) {
    let urlp = parseBitUrl(str)
    let drive = await BitStruct.get(urlp.hostname, {expect: 'bitdrive'})
    return { fs: drive.api, path: urlp.pathname, url: urlp, raw: str, isBit: true }
  }
  return { path: ''+str, raw: str, isBit: false }
}
...
async function createTarget (source) {
  ...
  let drive = await BitStruct.create('bitdrive')
